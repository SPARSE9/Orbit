...
// Lightweight reusable EventCard used by map views
    ...
    <Card className="p-4 bg-card border-accent/30">
      <div className="flex items-start justify-between mb-2">
        <div>
          ...
          <p className="text-xs text-muted-foreground">{event.category} • {new Date(event.date).toLocaleDateString()}</p>
        </div>
      ...
      <div className="text-sm text-muted-foreground">
        <p>{event.location}</p>
        <p className="mt-1">{event.attendees} attending</p>
      </div>
    </Card>
  )
}
